...
void Fun4All_Intt_Combiner (int       nEvents   = 10000,   //* 0 means process all the events
                            ...
                            const bool stripRawHit = false   /// Do not turn stripRawHit on, otherweise you got nothing...
) {
    vector<string> infile = {"intt0.list", "intt1.list", "intt2.list",
			     "intt3.list", "intt4.list", "intt5.list",
			     "intt6.list", "intt7.list"};
	...
	//! [TO CONFIRM] The switches for the ideal and survey geometries do not
	//! seem to work anymore. Is this intended?
    ...
    Fun4AllOutputManager *out = new Fun4AllDstOutputManager(
	"out", Form("%s/%s", outdirinitial.Data(), outfilename.Data()));
    if (stripRawHit) {
