...
int Fun4All_Clustering (int       run_num = 39526,
                        int       nEvents = 0,
                        int       job_num = 0,
                        const int skip    = 0) {
    ...
    // inputFile += "_no_hot_" + to_string (job_num) + ".root";
    inputFile += "_no_hot.root";

    std::string outputFile = kIntt_dst_dir + to_string (run_num) + "/" + "DST_cosmics_intt_" + run_num_str;
    if ( nEvents != 0 ) outputFile += "_" + std::to_string (nEvents) + "events";

    outputFile += "_no_hot_clusterized";
    // outputFile += to_string (job_num) + ".root";
    outputFile += ".root";
