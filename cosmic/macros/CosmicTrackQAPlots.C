...
TH1D *h1_angle_zr_distance_cut   = new TH1D ("h1_angle_zr_distance_cut_2", "angle dist.", 90, 0, 180);
TH1D *h1_angle_zr_distance_cut_1 = new TH1D ("h1_angle_zr_distance_cut_1", "angle dist.", 90, 0, 180);
TH1D *h1_angle_zr_distance_cut_2 = new TH1D ("h1_angle_zr_distance_cut_0.5", "angle dist.", 90, 0, 180);
TH1D *h1_angle_cluster_zr        = new TH1D ("h1_angle_cluster_zr", "cluster angle dist. in zr", 360, 0, 360);

TH2D *h2_angle_xy_zr = new TH2D ("h2_angle_xy_zr", "Angle correlation b/w xy & zr", 90, 0, 180, 90, 0, 180);
...
TH1D *h1_resi_xy_5clus   = new TH1D ("h1_resi_xy_5clus", "residual dist.", 1000, 0, 5);
...
TH2D *projection_hitmap_xy = new TH2D ("projection_hitmap_xy", "The projection point of origin in xy", 240, -12, 12, 240, -12, 12);
TH2D *projection_hitmap_zr = new TH2D ("projection_hitmap_zr", "The projection point of origin in zr", 480, -25, 25, 480, -25, 25);
TH2D *angle_distance_xy    = new TH2D ("angle_distance_xy", "Angle vs projection distance in xy", 90, 0, 180, 50, 0, 15);
TH2D *angle_distance_zr    = new TH2D ("angle_distance_zr", "Angle vs projection distance in zr", 90, 0, 180, 80, 0, 25);

//* distance from origin
TH1D *distance_from_origin_xy = new TH1D ("distance_from_origin_xy", "The distance from the origin in xy", 100, 0, 15);
TH1D *distance_from_origin_zr = new TH1D ("distance_from_origin_zr", "The distance from the origin in zr", 100, 0, 15);
...
void filling (string path   = "/sphenix/tg/tg01/commissioning/INTT/work/weiche/my_INTT/cosmic/DST_files/QA/cosmics/2024/root/cosmics_intt_00025145.root",
    ...
    tuple<double, double> projection_pos_zr = {0, 0};

    double projection_pos_x = 0;
    double projection_pos_y = 0;
    double projection_pos_z = 0;
    double projection_pos_r = 0;

    int  processing_event     = 0;
    int  pre_processing_event = 0;
    ...
    for ( int i = 0; i < loop_num; i++ ) {
        ...
        switch ( switchcase ) {
            //* switch case = 1, various plots
            case 1:
                //* initialize
                posz_total  = 0;
                distance_xy = 0;

                //* checking the coverage on z axis
                for ( unsigned int j = 0; j < n_cluster; j++ ) {
                ...
                ratio = (cover_counts / n_cluster) * 100;
                ...
                //* calculation for DCA
                distance_xy       = TMath::Abs (constant_xy) / TMath::Sqrt (1 + TMath::Power (slope_xy, 2));
                distance_zr       = TMath::Abs (constant_zr) / TMath::Sqrt (1 + TMath::Power (slope_zr, 2));
                projection_pos_x  = -(constant_xy) / (slope_xy + 1 / slope_xy);
                projection_pos_z  = -(constant_zr) / (slope_zr + 1 / slope_zr);
                projection_pos_y  = (constant_xy) / (TMath::Power (slope_xy, 2) + 1);
                projection_pos_r  = (constant_zr) / (TMath::Power (slope_zr, 2) + 1);
                projection_pos_xy = {projection_pos_x, projection_pos_y};
                projection_pos_zr = {projection_pos_z, projection_pos_r};

                //* filling histogram
                if ( total_cluster == 4 ) {
                    if ( residual_xy < 0.02 && is_coverage == true ) {
                        h1_angle_xy->Fill (angle_xy);
                        if ( distance_xy < 2 ) h1_angle_xy_distance_cut->Fill (angle_xy);
                        if ( distance_xy < 1 ) h1_angle_xy_distance_cut_1->Fill (angle_xy);
                        if ( distance_xy < 0.5 ) h1_angle_xy_distance_cut_2->Fill (angle_xy);
                        distance_from_origin_xy->Fill (distance_xy);
                        projection_hitmap_xy->Fill (projection_pos_x, projection_pos_y);
                        angle_distance_xy->Fill (angle_xy, distance_xy);
                        ...
                        if ( distance_zr < 2 ) h1_angle_zr_distance_cut->Fill (angle_zr);
                        if ( distance_zr < 1 ) h1_angle_zr_distance_cut_1->Fill (angle_zr);
                        if ( distance_zr < 0.5 ) h1_angle_zr_distance_cut_2->Fill (angle_zr);
                        distance_from_origin_zr->Fill (distance_zr);
                        projection_hitmap_zr->Fill (projection_pos_z, projection_pos_r);
                        angle_distance_zr->Fill (angle_zr, distance_zr);
                        ...
                        h2_angle_xy_zr->Fill (angle_xy, angle_zr);
                    }
                } else {
                    if ( track_accepted == true ) {
                        if ( processing_event != pre_processing_event ) track_accepted = false;
                        if ( processing_event == pre_processing_event ) track_accepted = true;
                    }
                    if ( track_accepted == false ) {
                        if ( residual_xy < 0.02 && is_coverage == true ) {
                            h1_angle_xy->Fill (angle_xy);
                            if ( distance_xy < 2 ) h1_angle_xy_distance_cut->Fill (angle_xy);
                            if ( distance_xy < 1 ) h1_angle_xy_distance_cut_1->Fill (angle_xy);
                ...
                if ( residual_xy < 0.02 && residual_zr < 0.2 && angle_zr < 92 && angle_zr > 88 && angle_xy < 92 && angle_xy > 88 ) {
                    for ( long unsigned int k = 0; k < posZ->size(); k++ ) {
        ...
        pre_processing_event = processing_event;
...
void CosmicTrackQAPlots (string file = "cosmics_intt_00039525_", string subdir = "39525", bool is_debug = 0, int c = 1, int q = 0) {
    gErrorIgnoreLevel = kSysError;

    string dir  = "/sphenix/u/wctang/workspace/my_INTT/cosmic/DST_files/QA/cosmics/2024/root/";
    // string dir  = "/sphenix/u/wctang/workspace/my_INTT/cosmic/macros/test/root/";
    ...
    string output_1        = dir + subdir + "/" + "plots_angle_zr_residaul_openup_multiclus_" + file + to_string (q) + ".root";
    ...
    if ( c == 1 ) {
        ...
        h1_angle_zr->Write();
        h1_angle_zr_distance_cut->Write();
        h1_angle_zr_distance_cut_1->Write();
        h1_angle_zr_distance_cut_2->Write();
        ...
        distance_from_origin_xy->Write();
        projection_hitmap_xy->Write();
        angle_distance_xy->Write();
        distance_from_origin_zr->Write();
        projection_hitmap_zr->Write();
        angle_distance_zr->Write();
        h2_angle_xy_zr->Write();
    } else if ( c == 2 ) {
