...
void ShowHelp() {
	...
	<< "+------------------------------------------------------------------"
	   "----------------------------------+"
	<< "| This is a Fun4All macro to produce a DST files using only INTT "
	   "data for commissioning.             |\n"
	<< "| It finds all root files using given run number.                  "
	   "                                  |\n"
	<< "| Warning: Data year (" << kYear
	<< ") is hard-corded in constant_values.hh. Change it by hand.      |\n"
	<< "|                                                                  "
	   "                                  |\n"
	<< "| void Fun4All_Intt_Combiner_run_base( int run_num, int nEvents, "
	   "bool is_debug = true )              |\n"
	<< "+------------------------------------------------------------------"
	   "----------------------------------+\n"
	<< endl;
...
void Fun4All_Intt_Combiner_run_base(int run_num = 39525, int nEvents = 10000,
				    bool use_cdb = false,
				    bool is_debug = true) {
    if (is_debug == true) ShowHelp();
    ...
    string output_dst =
	(is_debug ? "./" : kIntt_dst_dir) + debug_prefix + "DST" + output_base;

    if (use_cdb == true) output_dst += "_no_hot";
	...
	SingleInttPoolInput *sngl =
	    new SingleInttPoolInput("INTT_" + to_string(i));
    ...
    if (is_trkr_hit_on) {
	InttCombinedRawDataDecoder *myDecoder =
	    new InttCombinedRawDataDecoder("myUnpacker");
	myDecoder->runInttStandalone(true);
	myDecoder->writeInttEventHeader(true);

	if (use_cdb == true) {
	    string cdbttree_name =
		kIntt_cdb_dir + "cdb_" + to_string(run_num) + ".root";
	    myDecoder->LoadHotChannelMapLocal(
		cdbttree_name);	 /// For adding the hotchannel CDB

	    // string cdbttree_name_bco = cdb_output_path + "cdb_bco_" +
	    // to_string(run_num) + ".root";
	    string cdbttree_name_bco =
		kIntt_cdb_dir + "cdb_bco_" + to_string(run_num) + ".root";
	    myDecoder->SetCalibBCO(cdbttree_name_bco,
				   InttCombinedRawDataDecoder::FILE);
	}
