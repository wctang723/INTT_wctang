 ...
 */
void InttChannelClassifier_cosmics(int runnumber = 39525)  // runnumber
{
    ////////////////////////////////////////
    // Load HitMap                        //
    ////////////////////////////////////////
...
void aaaa(int runnumber) {
	...
	kIntt_hitmap_dir + "hitmap_run" + run_num_str + ".root";
    TFile *file = nullptr;
    ...
    TH1D *h1_hist_MPV_ladder =
	new TH1D("NumberOfFiredChannels_ladder", "NumberOfFiredChannels_ladder",
		 350, 1, 3501);
	...
	for (int j = 0; j < 14; j++) {
	    ...
	    h2_ColdMap[i][j] =
		new TH2D(Form("ColdMap_%d_%d", i, j),
			 Form("ColdMap_%d_%d", i, j), 128, 0, 128, 27, 0, 27);
	    h2_HalfMap[i][j] =
		new TH2D(Form("HalfMap_%d_%d", i, j),
			 Form("HalfMap_%d_%d", i, j), 128, 0, 128, 27, 0, 27);
	    h2_HotMap[i][j] =
		new TH2D(Form("HotMap_%d_%d", i, j), Form("HotMap_%d_%d", i, j),
			 128, 0, 128, 27, 0, 27);
	    h2_DeadMap[i][j] =
		new TH2D(Form("DeadMap_%d_%d", i, j),
			 Form("DeadMap_%d_%d", i, j), 128, 0, 128, 27, 0, 27);
	}
    }
    ...
    double par_meanA[8][14] = {0.};   // Array to save the mean & sigma value,
				      // [0][module] = mean, [1][module] = sigma
    double par_sigmaA[8][14] = {0.};  // Array to save the mean & sigma value,
				      // [0][module] = mean, [1][module] = sigma
    double par_meanB[8][14] = {0.};   // Array to save the mean & sigma value,
				      // [0][module] = mean, [1][module] = sigma
    double par_sigmaB[8][14] = {0.};  // Array to save the mean & sigma value,
				      // [0][module] = mean, [1][module] = sigma
    ...
    // std::string outputfile = root_output_path +
    // "InttHotDeadMap_"+to_string(runnumber)+"_"+to_string(in_sig)+".root";
    std::string outputfile = kIntt_hitmap_dir + "plots/" + "InttHotDeadMap_" +
			     run_num_str + "_" + to_string(in_sig) + ".root";

    // TFile *sfile = new TFile(Form("./rootfile/InttHotDeadMap_%d_%d.root",
    // runnumber, in_sig), "RECREATE");
    ...
    TCanvas *canB[8];
    for (int i = 0; i < 8; i++) {
	canA[i] = new TCanvas(Form("TypeA_Felix_%d", i),
			      Form("TypeA_Felix_%d", i), 1200, 1200);
	canB[i] = new TCanvas(Form("TypeB_Felix_%d", i),
			      Form("TypeB_Felix_%d", i), 1200, 1200);
	canA[i]->Divide(7, 2);
	canB[i]->Divide(7, 2);
    }
	...
	for (int i = 0; i < 14; i++) {
	    h1_hist_fit_A[felix][i] =
		new TH1D(Form("h1_hist_fit_A%d_%d", felix, i),
			 Form("h1_hist_fit_A%d_%d", felix, i), 200, 1, 201);
	    h1_hist_fit_B[felix][i] =
		new TH1D(Form("h1_hist_fit_B%d_%d", felix, i),
			 Form("h1_hist_fit_B%d_%d", felix, i), 200, 1, 201);
		...
		for (int chan = 0; chan < 128; chan++) {
		    int NumberOfEntry =
			h2_AllMap[felix][i]->GetBinContent(chan + 1, j + 1);
		    if (NumberOfEntry != 0) {
			NumberOfFiredChan[felix][i]++;
			auto content =
			    h2_AllMap[felix][i]->GetBinContent(chan + 1, j + 1);
			if (j < 5 ||
			    (j > 12 && j < 18))	 // Condition for type B
			{
    ...
    int hotchannelcut =
	h1_hist_MPV->GetXaxis()->GetBinCenter(h1_hist_MPV->GetMaximumBin()) * 2;
	...
	for (int i = 0; i < 14; i++) {
	    ...
	    //  HotChannelCut_A_Fit[felix][i] << endl; cout << "HELLO moudle : "
	    //  << i << " Type B " << HotChannelCut_B_Fit[felix][i] << endl;

	    for (int j = 0; j < 26; j++) {
		//    cout << "Felix : " << felix << " moudle : " << i << " Type
		//    A and chip : " << j << "  " <<
		...
		for (int chan = 0; chan < 128; chan++) {
			...
			NumberOfFiredChan[felix][i] > hotladdercut)
		    // if (ch_entry > hotchannelcut)
		    {
			flag_ = 8;
			...
			++size;
			h2_HotMap[felix][i]->SetBinContent(chan + 1, j + 1,
							   ch_entry);
		    }
		    // else if ( (felix==1 && module_ == 10) || (felix==1 &&
		    // module_<3) )
		    // {
		    //   flag_ = 8;
		    //   cdbttree->SetIntValue(size, "felix_server", felix);
