...
CosmicTracking::CosmicTracking(const std::string &name, const std::string &output_name) : SubsysReco(name) {
...
int CosmicTracking::Init(PHCompositeNode *topNode) {
    ...
    //g_hitmap_ = new TH2D("hist_chiphit_2Dmap_intt", "hist_chiphit_2Dmap_intt", 1000, 0, 1000, 1000, 0, 1000);
    //g_hitmap_->SetStats(0);
    //g_hitmap_->SetMaximum(1000000);
...
int CosmicTracking::process_event(PHCompositeNode *topNode) {
    ...
    if (status == Fun4AllReturnCodes::ABORTEVENT) return Fun4AllReturnCodes::ABORTEVENT;
    ...
    //  if (clusters_pair.size() > 10 || clusters_pair.size() < 3) return Fun4AllReturnCodes::EVENT_OK;

    // analyze events having 4 <= #cluster <= 5 only
    if (clusters.size() < 4 || clusters.size() > 5) return Fun4AllReturnCodes::EVENT_OK;
    ...
    */
    ...
    // if (*max_element(distances.begin(), distances.end()) < 8) return Fun4AllReturnCodes::EVENT_OK;
    // if (!(IsYFired_[0] && IsYFired_[1])) return Fun4AllReturnCodes::EVENT_OK;
...
//____________________________________________________________________________..
void CosmicTracking::Print(const std::string &what) const {
	std::cout << "CosmicTracking::Print(const std::string &what) const Printing info for " << what << std::endl;
}
...
std::string CosmicTracking::Int2Coordinate(int num) {
    if (num == 0) return "x";
...
bool CosmicTracking::IsFittable(TGraphErrors *g) {
    ...
    */
    ...
    for (unsigned int i = 1; i < values.size(); i++) {
        ...
        if (fabs(values[0] - values[i]) >= 0.16) return true;
    }
